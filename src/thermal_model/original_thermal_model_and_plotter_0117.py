...
class Electrolyzer:
    ...
    def __init__(self,r_active = 368,r_margin =53,r_endplate=524,thickness_plate=10.4,thickness_electrode = 1,thickness_insulator=3.75,thickness_endplate = 97, Power_rated = 125,current_density = 4000,interval = 20):
        ...
        self.n_cell = round( Power_rated * 1000/ 2.15 / (current_density * self.surf_area_active)) #integer, 34 is default value
        self.total_length = ((self.n_cell + 1) * thickness_plate + thickness_insulator *self.n_cell + thickness_endplate * 2) / 1000 #meters
        self.total_space = ((self.n_cell + 1) * 3.1415926 * ( (r_plate) ** 2 ) * thickness_plate + self.n_cell  * 3.1415926 * ( (r_plate) ** 2 ) * thickness_insulator + 3.1415926 * thickness_endplate * r_endplate ** 2) / 1000 ** 3 #cubic meters
        self.vacancy_space = (self.n_cell * 3.1415926 * r_active ** 2 * (thickness_plate - thickness_electrode)) /1000**3#cubic meters, room for alkaline
        self.occupied_space = self.total_space - self.vacancy_space - self.n_cell * 3.1415926 * (r_plate) ** 2 * thickness_insulator / 1000 ** 3  # cubic meters room for ss304
        self.rate_vacancy = self.vacancy_space / self.total_space #vacancy rate
        self.surface_area = (((self.n_cell + 1) * thickness_plate + self.n_cell * thickness_insulator) * 3.1415926 * 2 * (r_plate)
                             + 4 * 3.1415926 * r_endplate ** 2 - 2 * 3.1415926 * (r_plate) ** 2 + 2 * 3.1415926 * 2 * r_endplate * thickness_endplate) / 1000**2 # square meters, for radiation calculation
        self.interval = interval #s, the sampling interval of the model
        self.max_current_density = current_density


    def merge_coef(self,theta_power = 0,epsilon_electrolyzer = 0.95,rho_steel = 7900 ,rho_alkaline = 1280,HC_steel = 0.50 ,HC_alkaline = 2.2625):
        ...
        self.HC_alkaline_in_flux = 1000.0 / 3600.0 * rho_alkaline * HC_alkaline /1000. # kJ/s, can be directly timed with flux of alkaline and alkaline inlet temperature
        ...
        a = [ 9.76913600e-04, -3.19807956e-13,  5.66544937e-04, -6.41418079e-04]#目标模型的参数 1014
        ...
        if 0:#这一部分针对的如果模型需要重新进行参数矫正的话，才需要启用这一模块
            ...
            self.coef_radiation = corr_radiation * epsilon_electrolyzer * C_0 * self.surface_area /1000 / 100**4#kJ/s
            self.coef_HeatLyeIn = corr_HeatLyeIn * self.HC_alkaline_in_flux # kJ/s, can be directly timed with flux of alkaline and alkaline inlet temperature
            self.coef_HeatLyeOut = corr_HeatLyeOut * self.HC_alkaline_in_flux # kJ/s, can be directly timed with flux of alkaline and alkaline inlet temperature
            ...
            self.coef_radiation = self.corr_radiation * epsilon_electrolyzer * C_0 * self.surface_area /1000 / 100**4#kJ/s
            self.coef_HeatLyeIn = self.corr_HeatLyeIn * self.HC_alkaline_in_flux # kJ/s, can be directly timed with flux of alkaline and alkaline inlet temperature
            self.coef_HeatLyeOut = self.corr_HeatLyeOut * self.HC_alkaline_in_flux # kJ/s, can be directly timed with flux of alkaline and alkaline inlet temperature
    ...
    def Farady(self,current = current_default,Temp = T_default):
        ...
        f21 = 0.989
        f22 = 7.641E-5
        F_efficiency = (fake_current_density**2/(f11 - f12*Temp +fake_current_density**2))*(f21 + f22*Temp)
    ...
    def dT_calculation(self, T, ambT=ambT_default, Qlye=Qlye_default, Tlye=Tlye_default,  current=current_default):
        ...
        HeatLyeIn = Qlye * Tlye
        HeatLyeIOut = Qlye * T
        Model_input = [E * self.coef_power, Rad * self.coef_radiation, HeatLyeIn * self.coef_HeatLyeIn,
                       HeatLyeIOut * self.coef_HeatLyeOut]
        dT = sum(Model_input) / self.coef_DeltaTemp  # 这里就是要计算当前条件下的温度的变化
        return dT*self.interval

    def dT_calculation_adiabatic(self, T, ambT=ambT_default, Qlye=Qlye_default, Tlye=Tlye_default,  current=current_default):
        ...
        HeatLyeIn = Qlye * Tlye
        HeatLyeIOut = Qlye * T
        Model_input = [E * self.coef_power, HeatLyeIn * self.coef_HeatLyeIn,
                       HeatLyeIOut * self.coef_HeatLyeOut]
        dT = sum(Model_input) / self.coef_DeltaTemp  # 这里就是要计算当前条件下的温度的变化
        return dT*self.interval
    ...
    def T_thermal_eq(self, ambT: object = ambT_default, Qlye: object = Qlye_default, Tlye: object = Tlye_default,
                     current: object = current_default) -> object:
        ...
        while abs(dT) > 1E-8:
            ...
            if dT > 0 : T_left = T_mid
            if dT < 0 : T_right = T_mid
        return T_mid


    def T_thermal_eq_adiabatic(self, ambT: object = ambT_default, Qlye: object = Qlye_default, Tlye: object = Tlye_default,
                     current: object = current_default) -> object:
    ...
    def temp_flow(self,current_seq,Tlye_seq,T0 = 60,Qlye = Qlye_default):
        ...
        for i,n in enumerate(current_seq):
            ...
            dT = self.dT_calculation(T = T_seq[-1],ambT = 25, Qlye = Qlye, Tlye=Tlye_seq[i] ,current= n )
            dT_seq.append(dT)
            T_seq.append(T_seq[-1]+dT)
        return T_seq,dT_seq

    def polarizationcurve(self,current_max = 2000,ambT = ambT_default,Qlye = Qlye_default,Tlye = Tlye_default):
        ...
        return self.polar_current, self.polar_volatege,self.polar_power,self.polar_temp
    ...
    def Print_Model_coefs(self):
        coefs = [self.coef_power,self.coef_radiation ,self.coef_HeatLyeIn ,self.coef_HeatLyeOut ,self.coef_DeltaTemp]
...
def Polar_Curve_Tlye(current_max = 2000):
    ...
    plt.legend([r'$35\circ C$',r'$45\circ C$',r'$55\circ C$',r'$65\circ C$',r'$75\circ C$',r'$85\circ C$'],loc=7)
    #plt.title('Electrolyzer polarization curve under different lye input temperatures')
    plt.title('Electrolyzer outlet temperature under different current densities and lye input temperatures')
    plt.grid()
    plt.show()
...
def current_temp_farady():
    ...
    plt.legend([r'$35^\circ C$',r'$55^\circ C$',r'$75^\circ C$',r'$95^\circ C$'],loc=7)
    #plt.title('Electrolyzer polarization curve under different lye input temperatures')
    plt.grid()
    plt.show()
...
def current_TlyeIn_TlyeOut():
    ...
    for i in range(len(I_seq)):
        for j in range(len(Tlye_seq)):
            ...
            res_mat[j, i] = ele.T_thermal_eq(current=I_seq[i], Tlye=Tlye_seq[j], ambT=ambt) - Tlye_seq[j] # 这里就是画不同的工况下的平衡温度与入口温度的差值
    ...
    CS = plt.contourf(I_seq/ele.surf_area_active,Tlye_seq,res_mat, levels,
                     origin = 'upper',
                    cmap ='viridis',)
    ...
    print('the consumed time per point is', (time.time()- t0)/(len(I_seq)*len(Tlye_seq)))
    plt.show()
...
def current_TlyeIn_TlyeOut_adiabatic():
    ...
    for i in range(len(I_seq)):
        for j in range(len(Tlye_seq)):
            ...
            res_mat[j,i] = -(ele.T_thermal_eq_adiabatic(current=I_seq[i], Tlye=Tlye_seq[j]) *ele.coef_HeatLyeOut + Tlye_seq[j] * ele.coef_HeatLyeIn) * Qlye#这里是绝热状态下冷却功率
            res_mat[j, i] += (ele.T_thermal_eq(current=I_seq[i], Tlye=Tlye_seq[j], ambT=ambt) * ele.coef_HeatLyeOut + Tlye_seq[j] * ele.coef_HeatLyeIn) * Qlye# 绝热下冷却与15摄氏度冷却的差值
    plt.figure(figsize=(10,6))
    plt.subplots_adjust(left=0.074, bottom=0.095, right=1, top=0.967)
    '''这一部分主要就是各种画图的设置'''
    min_res =min(res_mat.flatten())
        ...
        for j in range(len(Tlye_seq)):
            ...
            while abs(dT) > 1E-2:
                ...
                if dT > 0:
                    ...
                    I_left = I_mid
            res[j] = I_mid * ele.polar(I_mid,Temp=Tlye_seq[j]) / 1000#进入口温度一样的时候，电解槽需要的输入功率
...
def current_TlyeIn_cooling():
    ...
    for i in range(len(I_seq)):
        for j in range(len(Tlye_seq)):
            ...
            res_mat[j, i] = (ele.T_thermal_eq(current=I_seq[i], Tlye=Tlye_seq[j], ambT=ambt) - Tlye_seq[j] )\
                            * (ele.coef_HeatLyeIn / ele.corr_HeatLyeIn)*Qlye_input
...
def cooling_Power_Qlye():
    ...
    for i in I_seq:
        ...
        for q in Qlye_seq:
            cooling_power = (ele.T_thermal_eq(current=i, Tlye=Tlye_input, ambT=ambt,Qlye=q) - Tlye_input )* \
                            (ele.coef_HeatLyeIn / ele.corr_HeatLyeIn)*q /125 *100#冷却功率需求
            T_eq = ele.T_thermal_eq(current=i, Tlye=Tlye_input, ambT=ambt,Qlye=q)#平衡温度
            res.append(T_eq)
...
def current_TlyeIn_Energy_cost_efficiency():
    ...
    for i in range(len(I_seq)):
        for j in range(len(Tlye_seq)):
            T_outlet_cur = ele.T_thermal_eq(Tlye=Tlye_seq[j],current=I_seq[i],ambT=ambt)
            FE,cd = ele.Farady(current=I_seq[i],Temp=T_outlet_cur)
            res_mat[j,i] = Vtn(T_outlet_cur)*ele.n_cell/ele.polar(current=I_seq[i],Temp=T_outlet_cur)*FE
    plt.figure(figsize=(10,6))
...
def current_TlyeIn_overalcost():
    ...
    for i in range(len(I_seq)):
        for j in range(len(Tlye_seq)):
            ...
            cooling_power= (T_eq - Tlye_seq[j]) * Qlye * (ele.coef_HeatLyeIn / ele.corr_HeatLyeIn)
...
def current_TlyeIn_liftcycle_cost():
    ...
    for i in range(len(I_seq)):
        for j in range(len(Tlye_seq)):
            ...
            cooling_power= (T_eq - tlye ) * Qlye * (ele.coef_HeatLyeIn / ele.corr_HeatLyeIn)
...
def Energy_flow_rad_heat():
    ...
    for i in range(len(I_seq)):
        for j in range(len(Tlye_seq)):
            T_eq = ele.T_thermal_eq(current=I_seq[i], Tlye=Tlye_seq[j],ambT=ambT)
            res_mat[j, i] = ele.coef_radiation * ((T_eq + 273.15)**4 - (ambT + 273.15)**4)
    plt.figure(figsize=(10, 6))
    plt.subplots_adjust(left=0.064, bottom=0.095, right=1, top=0.967)
    '''这一部分主要就是各种画图的设置'''
    min_res = min(res_mat.flatten())
